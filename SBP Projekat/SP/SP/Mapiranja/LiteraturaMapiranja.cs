...
namespace Studentski_projekti.Mapiranja
{
    class LiteraturaMapiranja:ClassMap<Literatura>
    {
        public LiteraturaMapiranja ()
        {
            ...
            HasMany(x => x.OsnovnaLiteratura).KeyColumn("OSNOVNA_LITERATURA.LITERATURA_ID").Inverse();
            HasMany(x => x.DodatnaLiteratura).KeyColumn("DODATNA_LITERATURA.LITERATURA_ID").Inverse();
